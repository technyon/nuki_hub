...
void WebCfgServer::buildCredHtml(AsyncWebServerRequest *request)
{
    _response = "";
    buildHtmlHeader();
    _response.concat("<form id=\"credfrm\" class=\"adapt\" onsubmit=\"testcreds()\" method=\"post\" action=\"savecfg\">");
    ...
    _response.concat("</table>");
    _response.concat("<br><input type=\"submit\" name=\"submit\" value=\"Save\">");
    _response.concat("</form><script>function testcreds() { var input_value = document.getElementById(\"inputpass\").value(); var pattern = /^[\x20-\x7E]*$/; if(!pattern.test(input_value)) { alert('Only non unicode characters are allowed in username and password'); return false;}</script>");
    if(_nuki != nullptr)
    {
