...
void NukiNetworkOpener::publishKeypad(const std::list<NukiOpener::KeypadEntry>& entries, uint maxKeypadCodeCount)
{
    uint index = 0;
    char uidString[20];
...
void NukiNetworkOpener::publishAuth(const std::list<NukiOpener::AuthorizationEntry>& authEntries, uint maxAuthEntryCount)
{
    ...
    serializeJson(json, _buffer, _bufferSize);
    publishString(mqtt_topic_auth_json, _buffer);
...
void NukiNetworkOpener::publishKeypadEntry(const String topic, NukiOpener::KeypadEntry entry)
{
    if(_preferences->getBool(preference_disable_non_json, false)) return;
